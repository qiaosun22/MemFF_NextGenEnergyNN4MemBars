...
class Weight_Quant_Noise(torch.autograd.Function):
    ...
    def forward(ctx, weight, bias, half_level, isint, clamp_std, noise_scale):
        ...
        # # log down the max scale for input weight
        # scale_in = abs(bias).max()

        # # log down the max scale for input weight
        # bias, scale = data_quantization_sym(bias, half_level, scale = scale_in,
        #                                       isint = isint, clamp_std = 0)
...
class Conv2d_quant_noise(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        # print("self.weight, self.bias", self.weight, self.bias)
...
class Linear_quant_noise(nn.Linear):
    ...
    def forward(self, x):
        ...
        self.counter += 1
        self.biasli.append(bias_q.cpu().detach().numpy())

        # print("weight_q, bias_q", weight_q, bias_q)
        x = F.linear(x, weight_q, bias_q)
        # x1 = x
        # print(x1)
        # np.save('x1.npy', x1.cpu().detach().numpy())
        # x = F.linear(x, self.weight, self.bias)
        # print(x)
        # np.save('x.npy', x.cpu().detach().numpy())
        # print(self.weight, self.bias, weight_q, bias_q)
        # np.save('weight.npy', self.weight.cpu().detach().numpy())
        # np.save('bias.npy', self.bias.cpu().detach().numpy())
        # np.save('weight_q.npy', weight_q.cpu().detach().numpy())
        # np.save('bias_q.npy', bias_q.cpu().detach().numpy())
